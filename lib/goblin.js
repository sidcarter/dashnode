...
//	console.log(fileType); 
	var path = pathName[1];
	var version = 'Running ' + process.title + ' ' + process.version + ' on ' + process.platform + '.';
	...
	switch(fileType) {
		case 'js' :
			fs.readFile(scriptDir + path, 'utf8', function(err,data){
				if (err) notFound(path, response);
				else renderJs(data,response);
			});
			break;

		case 'txt' :
			fs.readFile(viewDir + path, 'utf8', function(err,data){
				if (err) {
		...
		case 'jpeg' :
			fs.readFile(imageDir + path, function(err,data){
				if (err) notFound(err,response);
				else renderImage(data,response);
			});
			break;		
...
var notFound = function(path,response){
	...
	options.body = "<div class='status'>" + statusCode + 
	"</div> And I still haven't found what you're looking for: <span class='notFound'>" 
	+ path + "</span>";
	jade.renderFile(layoutFile, {locals: options}, function(err,data){
		if (err) console.log(err.stack);
		response.end(data);
